...
# Per-language UI strings used throughout the document
_DOC_STRINGS: dict[str, dict[str, str]] = {
    "es": {
        "draft_subtitle":   "Draft de documentación realizada por GenDoc",
        "rights":           "Todos los derechos reservados",
        "rights_footer":    "Todos los derechos reservados",
        "author_label":     "Autor(a):",
        "draft_label":      "Draft generado por GenDoc",
        "toc_agenda":       "Agenda",
        "toc_index":        "Índice",
        "toc_tables":       "Índice de Tablas",
        "tbl_toc_prefix":   "Tabla",
        "caption_table":    "Tabla",
        "caption_code":     "Código",
        "caption_diagram":  "Diagrama",
        "caption_default_table":   "Datos tabulares",
        "caption_default_code":    "Fragmento de código",
        "caption_default_diagram": "Diagrama del sistema",
        "diagram_label":    "[Diagrama Mermaid:]",
    },
    "en": {
        "draft_subtitle":   "Documentation draft generated by GenDoc",
        "rights":           "All rights reserved",
        "rights_footer":    "All rights reserved",
        "author_label":     "Author:",
        "draft_label":      "Draft generated by GenDoc",
        "toc_agenda":       "Agenda",
        "toc_index":        "Table of Contents",
        "toc_tables":       "List of Tables",
        "tbl_toc_prefix":   "Table",
        "caption_table":    "Table",
        "caption_code":     "Code",
        "caption_diagram":  "Diagram",
        "caption_default_table":   "Tabular data",
        "caption_default_code":    "Code snippet",
        "caption_default_diagram": "System diagram",
        "diagram_label":    "[Mermaid Diagram:]",
    },
}


def _ds(lang: str, key: str) -> str:
    """Return the document UI string for *key* in *lang*."""
    return _DOC_STRINGS.get(lang, _DOC_STRINGS["es"]).get(key, _DOC_STRINGS["es"][key])


def _month_year(lang: str = "es") -> str:
    now = datetime.now()
    if lang == "en":
        return f"{_MONTHS_EN[now.month - 1]} {now.year}"
...
def _prescan_elements(
    ...
    while i < n:
        ...
        if stripped == "[DIAGRAM]":
            diagram_num += 1
            bk_id       += 1
            desc    = pending_desc or _ds(lang, "caption_default_diagram")
            anchor  = f"diag_{diagram_num}"
            caption_map[i] = (_ds(lang, "caption_diagram"), desc, diagram_num, anchor, bk_id)
            pending_desc = None
            i += 1
        ...
        if "|" in line and stripped.startswith("|"):
            ...
            table_num += 1
            bk_id     += 1
            desc   = pending_desc or _ds(lang, "caption_default_table")
            anchor = f"tbl_{table_num}"
            caption_map[first_i] = (_ds(lang, "caption_table"), desc, table_num, anchor, bk_id)
        ...
        if line.startswith("```"):
            ...
            code_num += 1
            bk_id    += 1
            desc   = pending_desc or _ds(lang, "caption_default_code")
            anchor = f"code_{code_num}"
            caption_map[first_i] = (_ds(lang, "caption_code"), desc, code_num, anchor, bk_id)
            pending_desc = None
            continue
...
def _diagram_block(doc: Document, mermaid_code: str, secondary_rgb: RGBColor,
                   lang: str = "es") -> None:
    """
    ...
    label_run = label.add_run(_ds(lang, "diagram_label"))
...
def _add_cover_page(doc: Document, title: str,
    ...
    sub_run = sub_para.add_run(_ds(lang, "draft_subtitle"))
    ...
    rights_run = rights_para.add_run(_ds(lang, "rights"))
...
def _add_toc_page(
    ...
    *table_toc_entries* → (number, description, anchor_id) for the Índice de Tablas.
    ...
    if table_toc_entries:
        ...
        run_title = spacer.add_run(_ds(lang, "toc_tables"))
        ...
        for num, description, anchor in table_toc_entries:
            ...
            _add_toc_hyperlink(
                para,
                f"{_ds(lang, 'tbl_toc_prefix')} {num},  {description}",
                anchor,
                primary_hex,
                20,
                False,
...
def _setup_header_footer(doc: Document, project_title: str,
    ...
    r_author = p_author.add_run(_ds(lang, "author_label"))
    ...
    r_draft = p_draft.add_run(_ds(lang, "draft_label"))
    ...
    r_rights = fp.add_run(f"{_ds(lang, 'rights_footer')}, {month_year}")
...
def convert(
    ...
    *,
    ...
    caption_map, table_toc_entries, bk_counter = _prescan_elements(
        lines, title_line_idx, bk_counter, lang=output_lang
    )

    # ── Cover page (section 1, vAlign=center) ────────────────────────
    _add_cover_page(doc, title, primary_rgb, secondary_rgb, lang=output_lang)

    # ── Header + footer on main section ──────────────────────────────
    _setup_header_footer(doc, title, primary_rgb, secondary_rgb, lang=output_lang)
    ...
    # ── TOC / Agenda page ────────────────────────────────────────────
    toc_title = _ds(output_lang, "toc_agenda") if doc_type == "executive" else _ds(output_lang, "toc_index")
    if toc_entries or table_toc_entries:
        _add_toc_page(doc, toc_entries, toc_title,
                      ...
                      table_toc_entries, lang=output_lang)
    ...
    while i < n:
        ...
        if line.strip() == "[DIAGRAM]":
            ...
            while i < n and lines[i].strip() != "[/DIAGRAM]":
                ...
                i += 1
            _diagram_block(doc, "\n".join(diagram_lines), secondary_rgb, lang=output_lang)
